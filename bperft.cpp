...
void AddAttacksFrom(const int pc, const int from) {
  ...
  const int count = (pc < Queen) ? 4 : 8;
  Move** mvs = (pc < Rook)  ? _bishopMoves[from] :
...
public:
  ...
  bool AttackedBy(const int sqr) const {
    ...
    int dir;
    if (_atkd[sqr]) {
      for (uint64_t tmp = _atkd[sqr]; tmp; tmp >>= 8) {
        if (tmp & 0xFF) {
    ...
    for (mv = _kingMoves[sqr][color]; *mv; ++mv) {
      assert(mv->Valid());
      assert(mv->From() == sqr);
      if (mv->Type() == KingMove) {
        switch (_board[mv->To()]) {
        case (color|Pawn):
          dir = Direction(sqr, mv->To());

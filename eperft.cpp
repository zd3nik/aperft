...
inline void AddSlide(const int from, const int to, const int dir) {
  ...
  assert(IS_SLIDER(_board[from]));
  assert(Direction(from, to) == dir);
  assert(!(_slide[from] & (0xFFULL << DirShift(dir))) ||
         ((_slide[from] & (0xFFULL << DirShift(dir))) ==
          (uint64_t(to + 1) << DirShift(dir))));
  _slide[from] |= (uint64_t(to + 1) << DirShift(dir));
}
...
inline void SetSlide(const int from, const int to, const int dir) {
  ...
  _slide[from] = ((_slide[from] & ~(0xFFULL << shift)) |
                  (uint64_t(to + 1) << shift));
}
...
public:
  ...
  bool VerifySlide(uint64_t mvs, const int from, const bool do_assert) const {
    uint64_t slide = 0;
    while (mvs) {
      assert(mvs & 0xFF);
      const int end = ((mvs & 0xFF) - 1);
    ...
    if (do_assert) {
      assert(slide == _slide[from]);
    }
    return (slide == _slide[from]);
  }
  bool VerifyAttacks(const bool do_assert) const {
    for (int sqr = A1; sqr <= H8; ++sqr) {
      ...
      switch (Black|_board[sqr]) {
      case (Black|Bishop):
        if (!VerifySlide(_bishopMoves[sqr], sqr, do_assert)) {
          return false;
        }
        break;
      case (Black|Rook):
        if (!VerifySlide(_rookMoves[sqr], sqr, do_assert)) {
          return false;
        }
        break;
      case (Black|Queen):
        if (!VerifySlide(_queenMoves[sqr], sqr, do_assert)) {
          return false;
        }
        break;
      }
